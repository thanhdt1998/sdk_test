...
const CGFloat VMFrameTimeExaggeration = 4.f * 1000.f;
const CGFloat VMFrameTimeBarWidth = 4.f;

@interface VMFrameTimeGraphView ()
...
@implementation VMFrameTimeGraphView
...
- (void)updatePathWithFrameDuration:(CFTimeInterval)frameDuration {
    ...
    self.currentX += VMFrameTimeBarWidth;
...
- (CGFloat)renderDurationTargetMilliseconds {
    ...
    CGFloat target = (1.0 / maximumFramesPerSecond) * VMFrameTimeExaggeration;
...
- (CAShapeLayer *)barWithFrameDuration:(CFTimeInterval)frameDuration {
    ...
    CGRect barRect = CGRectMake(0, 0, VMFrameTimeBarWidth, -(fminf(frameDuration * VMFrameTimeExaggeration, self.frame.size.height)));
...
- (UIColor *)colorForFrameDuration:(CFTimeInterval)frameDuration {
    ...
    frameDuration *= VMFrameTimeExaggeration;
...
- (void)removeStaleBars {
    if (self.barLayers.count > (self.frame.size.width / VMFrameTimeBarWidth * 3)) {
        NSRange staleBarsRange = NSMakeRange(0, self.frame.size.width / VMFrameTimeBarWidth);
