...
#import "VMCustomCalloutView.h"
#import "VMOfflinePacksTableViewController.h"
#import "VMAnnotationView.h"
#import "VMUserLocationAnnotationView.h"
...
#import "VMFrameTimeGraphView.h"
#import "VMMapView_Experimental.h"
...
static const VMGLCoordinateBounds colorado = {
    .sw = { .latitude = 36.986207, .longitude = -109.049896},
    .ne = { .latitude = 40.989329, .longitude = -102.062592},
};

static NSString * const VMViewControllerAnnotationViewReuseIdentifer = @"MBXViewControllerAnnotationViewReuseIdentifer";

typedef NS_ENUM(NSInteger, VMSettingsSections) {
    VMSettingsDebugTools = 0,
    VMSettingsAnnotations,
    VMSettingsRuntimeStyling,
    VMSettingsMiscellaneous,
};

typedef NS_ENUM(NSInteger, VMSettingsDebugToolsRows) {
    VMSettingsDebugToolsResetPosition = 0,
    VMSettingsDebugToolsTileBoundaries,
    VMSettingsDebugToolsTileInfo,
    VMSettingsDebugToolsTimestamps,
    VMSettingsDebugToolsCollisionBoxes,
    VMSettingsDebugToolsOverdrawVisualization,
    VMSettingsDebugToolsShowZoomLevel,
    VMSettingsDebugToolsShowFrameTimeGraph,
    VMSettingsDebugToolsShowReuseQueueStats
};
...
@interface VMDroppedPinAnnotation : VMPointAnnotation
@end

@implementation VMDroppedPinAnnotation
@end

@interface VMCustomCalloutAnnotation : VMPointAnnotation
@property (nonatomic, assign) BOOL anchoredToAnnotation;
@property (nonatomic, assign) BOOL dismissesAutomatically;
@end

@implementation VMCustomCalloutAnnotation
@end

@interface VMSpriteBackedAnnotation : VMPointAnnotation
@end

@implementation VMSpriteBackedAnnotation
@end

@interface VMViewController () <UITableViewDelegate,
                                 UITableViewDataSource,
                                 VMGLMapViewDelegate,
                                 VMComputedShapeSourceDataSource>


@property (nonatomic) IBOutlet VMGLMapView *mapView;
@property (nonatomic) VMState *currentState;
@property (weak, nonatomic) IBOutlet UIButton *hudLabel;
@property (weak, nonatomic) IBOutlet VMFrameTimeGraphView *frameTimeGraphView;
...
@interface VMGLMapView (VMViewController)
...
- (void)viewDidLoad
{
    ...
    self.currentState =  [VMStateManager sharedManager].currentState;

    if (!self.currentState) {
        // Create a new state with the below default values
        self.currentState = [[VMState alloc] init];
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:UIScreenDidConnectNotification object:nil queue:nil usingBlock:^(NSNotification * _Nonnull note) {
        ...
        VMGLMapView *helperMapView = [[VMGLMapView alloc] initWithFrame:helperWindow.bounds styleURL:[[VMGLStyle predefinedStyle:@"Hybrid"] url]];
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(__unused id)sender {
    if ([segue.identifier isEqualToString:@"ShowOfflinePacks"]) {
        VMOfflinePacksTableViewController *controller = [segue destinationViewController];
...
- (NSArray <NSString *> *)settingsTitlesForSection:(NSInteger)section
{
    ...
    VMGLMapDebugMaskOptions debugMask = self.mapView.debugMask;

    switch (section)
    {
        case VMSettingsDebugTools:
            [settingsTitles addObjectsFromArray:@[
                @"Reset position",
                [NSString stringWithFormat:@"%@ tile boundaries",
                    (debugMask & VMGLMapDebugTileBoundariesMask ? @"Hide" :@"Show")],
                [NSString stringWithFormat:@"%@ tile info",
                    (debugMask & VMGLMapDebugTileInfoMask ? @"Hide" :@"Show")],
                [NSString stringWithFormat:@"%@ tile timestamps",
                    (debugMask & VMGLMapDebugTimestampsMask ? @"Hide" :@"Show")],
                [NSString stringWithFormat:@"%@ collision boxes",
                    (debugMask & VMGLMapDebugCollisionBoxesMask ? @"Hide" :@"Show")],
                [NSString stringWithFormat:@"%@ overdraw visualization",
                    (debugMask & VMGLMapDebugOverdrawVisualizationMask ? @"Hide" :@"Show")],
...
- (void)performActionForSettingAtIndexPath:(NSIndexPath *)indexPath
{
    switch (indexPath.section)
    {
        case VMSettingsDebugTools:
            ...
            {
                case VMSettingsDebugToolsResetPosition:
                    [self.mapView resetPosition];
                    break;
                case VMSettingsDebugToolsTileBoundaries:
                    self.currentState.debugMask ^= VMGLMapDebugTileBoundariesMask;
                    break;
                case VMSettingsDebugToolsTileInfo:
                    self.currentState.debugMask ^= VMGLMapDebugTileInfoMask;
                    break;
                case VMSettingsDebugToolsTimestamps:
                    self.currentState.debugMask ^= VMGLMapDebugTimestampsMask;
                    break;
                case VMSettingsDebugToolsCollisionBoxes:
                    self.currentState.debugMask ^= VMGLMapDebugCollisionBoxesMask;
                    break;
                case VMSettingsDebugToolsOverdrawVisualization:
                    self.currentState.debugMask ^= VMGLMapDebugOverdrawVisualizationMask;
                    break;
                case VMSettingsDebugToolsShowZoomLevel:
                {
                ...
                case VMSettingsDebugToolsShowFrameTimeGraph:
                {
                ...
                case VMSettingsDebugToolsShowReuseQueueStats:
                {
        ...
        case VMSettingsMiscellaneous:
            switch (indexPath.row)
            {
                ...
                case MBXSettingsMiscellaneousScrollView:
                {
                    ...
                    VMEmbeddedMapViewController *embeddedMapViewController = (VMEmbeddedMapViewController *)[storyboard instantiateViewControllerWithIdentifier:@"MBXEmbeddedMapViewController"];
                ...
                case MBXSettingsMiscellaneousSetContentInsets:
                {
                    ...
                    if (_contentInsetsEnabled) {
                        ...
                        VMGLMapCamera *camera = [VMGLMapCamera cameraLookingAtCenterCoordinate:CLLocationCoordinate2DMake(39.72707, -104.9986)
                                                                              acrossDistance:100
                                                                                       pitch:60
                                                                                     heading:0];
                        __weak VMViewController *weakSelf = self;
                ...
                {
                    VMOrnamentsViewController *ornamentsViewController = [[VMOrnamentsViewController alloc] init];
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^
    {
        ...
        if ([features isKindOfClass:[NSDictionary class]])
        {
            ...
            for (NSDictionary *feature in features[@"features"])
            {
                ...
                VMPointAnnotation *annotation = (useViews ? [VMPointAnnotation new] : [VMSpriteBackedAnnotation new]);
...
- (void)animateAnnotationView
    {
        VMPointAnnotation *annot = [[VMPointAnnotation alloc] init];
...
- (void)addTestShapes
{
    ...
    VMGLPolygon *triangle = [VMGLPolygon polygonWithCoordinates:triangleCoordinates count:3];
    ...
    VMGLPolyline *line = [VMGLPolyline polylineWithCoordinates:lineCoordinates count:4];
    ...
    VMGLPolyline *polyline = [VMGLPolyline polylineWithCoordinates:polylineCoordinates
    ...
    for (NSDictionary *feature in threestates[@"features"])
    {
        ...
        VMGLPolygon *polygon = [VMGLPolygon polygonWithCoordinates:polygonCoordinates count:[stateCoordinatePairs count]];
    ...
    VMGLPolygon *innerPolygon = [VMGLPolygon polygonWithCoordinates:innerCoordinates count:sizeof(innerCoordinates) / sizeof(innerCoordinates[0])];
    ...
    VMGLPolygon *outerPolygon = [VMGLPolygon polygonWithCoordinates:outerCoordinates count:sizeof(outerCoordinates) / sizeof(outerCoordinates[0]) interiorPolygons:@[innerPolygon]];
...
- (void)addAnnotationWithCustomCallout
{
    ...
    VMCustomCalloutAnnotation *firstAnnotation = [[VMCustomCalloutAnnotation alloc] init];
    ...
    VMCustomCalloutAnnotation *secondAnnotation = [[VMCustomCalloutAnnotation alloc] init];
    ...
    VMCustomCalloutAnnotation *thirdAnnotation = [[VMCustomCalloutAnnotation alloc] init];
...
{
    VMGLSource* source = [self.mapView.style sourceWithIdentifier:@"composite"];
    if (source) {
...
- (void)styleQuery
{
    ...
    VMGLSource *source = [self.mapView.style sourceWithIdentifier:querySourceID];
    if (source) {
...
- (void)styleFeature
{
    ...
    VMGLShape *shape = [VMGLShape shapeWithData:data encoding:NSUTF8StringEncoding error:NULL];
    ...
    VMGLPolygon *polygon = [VMGLPolygon polygonWithCoordinates:squareCoords count:sizeof(squareCoords)/sizeof(squareCoords[0])];
...
- (void)updateShapeSourceData
{
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        VMGLShape *updatedShape = [VMGLShape shapeWithData:updatedData encoding:NSUTF8StringEncoding error:NULL];
...
- (void)styleRouteLine
{
    ...
    VMGLPolylineFeature *routeLine = [VMGLPolylineFeature polylineWithCoordinates:coords count:count];
...
- (IBAction)startWorldTour
{
    ...
    {
        VMDroppedPinAnnotation *annotation = [[VMDroppedPinAnnotation alloc] init];
...
- (void)continueWorldTourWithRemainingAnnotations:(NSMutableArray<VMPointAnnotation *> *)annotations
{
    VMPointAnnotation *nextAnnotation = annotations.firstObject;
    ...
    VMGLMapCamera *camera = [VMGLMapCamera cameraLookingAtCenterCoordinate:nextAnnotation.coordinate
                                                          acrossDistance:10
                                                                   pitch:arc4random_uniform(60)
                                                                 heading:arc4random_uniform(360)];
    __weak VMViewController *weakSelf = self;
    [self.mapView flyToCamera:camera completionHandler:^{
        VMViewController *strongSelf = weakSelf;
...
- (void)toggleCustomUserDot
{
    ...
    self.mapView.userTrackingMode = VMGLUserTrackingModeFollow;
}
...
- (id<VMGLAnnotation>)randomOffscreenPointAnnotation {
    ...
    NSPredicate *pointAnnotationPredicate = [NSPredicate predicateWithBlock:^BOOL(id  _Nullable evaluatedObject, NSDictionary<NSString *,id> * _Nullable bindings) {
        return [evaluatedObject isKindOfClass:[VMPointAnnotation class]];
    }];
...
- (void)selectAnOffscreenPointAnnotation {
    id<VMGLAnnotation> annotation = [self randomOffscreenPointAnnotation];
    if (annotation) {
...
- (void)centerSelectedAnnotation {
    id<VMGLAnnotation> annotation = self.mapView.selectedAnnotations.firstObject;
...
- (void)addVisibleAreaPolyline {
    ...
    VMGLPolyline *line = [VMGLPolyline polylineWithCoordinates:lineCoords
...
- (UIImage *)mapView:(VMGLMapView *)mapView didFailToLoadImage:(NSString *)imageName {
...
- (void)randomWorldTourInternal {
    ...
    VMGLMapCamera *camera = [VMGLMapCamera cameraLookingAtCenterCoordinate:annotation.coordinate
                                                                altitude:10000.0
...
- (void)toggleSecondMapView {
    if ([self.view viewWithTag:2] == nil) {
        VMGLMapView *secondMapView = [[VMGLMapView alloc] initWithFrame:
        ...
        VMGLMapView *secondMapView = (VMGLMapView *)[self.view viewWithTag:2];
        for (NSLayoutConstraint *constraint in self.view.constraints)
        {
...
- (IBAction)handleLongPress:(UILongPressGestureRecognizer *)longPress
{
    if (longPress.state == UIGestureRecognizerStateBegan)
    {
        ...
        NSString *title;
        for (id <VMGLFeature> feature in features) {
            if (!title) {
                title = [feature attributeForKey:@"name_en"] ?: [feature attributeForKey:@"name"];
            }
        }

        VMDroppedPinAnnotation *pin = [[VMDroppedPinAnnotation alloc] init];
...
- (void)setStyles
{
    ...
    NSString* apiKey = [VMGLSettings apiKey];
    if (apiKey.length)
    {
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            
            for (MGLDefaultStyle* predefinedStyle in [VMGLStyle predefinedStyles]){
...
- (VMGLAnnotationView *)mapView:(VMGLMapView *)mapView viewForAnnotation:(id<VMGLAnnotation>)annotation
{
    if (annotation == mapView.userLocation)
    {
        if (_customUserLocationAnnnotationEnabled)
        {
            VMUserLocationAnnotationView *annotationView = [[VMUserLocationAnnotationView alloc] initWithFrame:CGRectZero];
    ...
    if ([annotation isKindOfClass:[VMDroppedPinAnnotation class]] || [annotation isKindOfClass:[VMSpriteBackedAnnotation class]])
    {
        return nil;
    }

    VMAnnotationView *annotationView = (VMAnnotationView *)[mapView dequeueReusableAnnotationViewWithIdentifier:VMViewControllerAnnotationViewReuseIdentifer];
    if (!annotationView)
    {
        annotationView = [[VMAnnotationView alloc] initWithReuseIdentifier:VMViewControllerAnnotationViewReuseIdentifer];
...
- (VMGLAnnotationImage *)mapView:(VMGLMapView * __nonnull)mapView imageForAnnotation:(id <VMGLAnnotation> __nonnull)annotation
{
    if ([annotation isKindOfClass:[VMDroppedPinAnnotation class]] || [annotation isKindOfClass:[VMCustomCalloutAnnotation class]])
    {
        return nil; // use default marker
    }

    NSAssert([annotation isKindOfClass:[VMSpriteBackedAnnotation class]], @"Annotations should be sprite-backed.");

    NSString *title = [(VMPointAnnotation *)annotation title];
    if (!title.length) return nil;
    NSString *lastTwoCharacters = [title substringFromIndex:title.length - 2];

    VMGLAnnotationImage *annotationImage = [mapView dequeueReusableAnnotationImageWithIdentifier:lastTwoCharacters];
    ...
    if ( ! annotationImage)
    {
        ...
        annotationImage = [VMGLAnnotationImage annotationImageWithImage:image reuseIdentifier:lastTwoCharacters];
...
- (BOOL)mapView:(__unused VMGLMapView *)mapView annotationCanShowCallout:(__unused id <VMGLAnnotation>)annotation
{
    return YES;
}

- (CGFloat)mapView:(__unused VMGLMapView *)mapView alphaForShapeAnnotation:(VMGLShape *)annotation
{
    return ([annotation isKindOfClass:[VMGLPolygon class]] ? 0.5 : 1.0);
}

- (UIColor *)mapView:(__unused VMGLMapView *)mapView strokeColorForShapeAnnotation:(VMGLShape *)annotation
{
    UIColor *color = [annotation isKindOfClass:[VMGLPolyline class]] ? [UIColor greenColor] : [UIColor blackColor];
    return [color colorWithAlphaComponent:0.9];
}

- (UIColor *)mapView:(__unused VMGLMapView *)mapView fillColorForPolygonAnnotation:(__unused VMGLPolygon *)annotation
{
    UIColor *color = annotation.pointCount > 3 ? [UIColor greenColor] : [UIColor redColor];
    return [color colorWithAlphaComponent:0.5];
}

- (void)mapView:(__unused VMGLMapView *)mapView didChangeUserTrackingMode:(VMGLUserTrackingMode)mode animated:(__unused BOOL)animated
{
...
- (nullable id <VMGLCalloutView>)mapView:(__unused VMGLMapView *)mapView calloutViewForAnnotation:(id<VMGLAnnotation>)annotation
{
    if ([annotation respondsToSelector:@selector(title)]
        && [annotation isKindOfClass:[VMCustomCalloutAnnotation class]])
    {
        VMCustomCalloutAnnotation *customAnnotation = (VMCustomCalloutAnnotation *)annotation;
        VMCustomCalloutView *calloutView = [[VMCustomCalloutView alloc] init];
...
- (UIView *)mapView:(__unused VMGLMapView *)mapView leftCalloutAccessoryViewForAnnotation:(__unused id<VMGLAnnotation>)annotation
{
...
- (UIView *)mapView:(__unused VMGLMapView *)mapView rightCalloutAccessoryViewForAnnotation:(__unused id<VMGLAnnotation>)annotation
{
    UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
    button.frame = CGRectZero;
...
- (void)mapView:(VMGLMapView *)mapView tapOnCalloutForAnnotation:(id <VMGLAnnotation>)annotation
{
    ...
    VMPointAnnotation *point = (VMPointAnnotation *)annotation;
...
- (BOOL)mapView:(VMGLMapView *)mapView shouldChangeFromCamera:(VMGLMapCamera *)oldCamera toCamera:(VMGLMapCamera *)newCamera {
    if (_shouldLimitCameraChanges) {
        // Get the current camera to restore it after.
        VMGLMapCamera *currentCamera = mapView.camera;
        ...
        VMGLCoordinateBounds newVisibleCoordinates = mapView.visibleCoordinateBounds;

        // Revert the camera.
        mapView.camera = currentCamera;

        // Test if the newCameraCenter and newVisibleCoordinates are inside Colorado.
        BOOL inside = VMGLCoordinateInCoordinateBounds(newCameraCenter, colorado);
        BOOL intersects = VMGLCoordinateInCoordinateBounds(newVisibleCoordinates.ne, colorado) && VMGLCoordinateInCoordinateBounds(newVisibleCoordinates.sw, colorado);
...
- (void)mapViewRegionIsChanging:(VMGLMapView *)mapView
{
    [self updateHUD];
    [self updateHelperMapViews];
}

- (void)mapView:(VMGLMapView *)mapView regionDidChangeWithReason:(VMGLCameraChangeReason)reason animated:(BOOL)animated
{
    if (reason != VMGLCameraChangeReasonProgrammatic) {
        self.randomWalk = NO;
    }
...
- (void)mapView:(VMGLMapView *)mapView didUpdateUserLocation:(VMGLUserLocation *)userLocation {
    [self updateHUD];
}

- (void)updateHelperMapViews {
    for (UIWindow *window in self.helperWindows) {
        VMGLMapView *mapView = (VMGLMapView *)window.rootViewController.view;
...
- (NSArray<id <VMGLFeature>>*)featuresInCoordinateBounds:(VMGLCoordinateBounds)bounds zoomLevel:(NSUInteger)zoom {
    double gridSpacing;
    if(zoom >= 13) {
        gridSpacing = 0.01;
    } else if(zoom >= 11) {
    ...
    NSMutableArray <id <VMGLFeature>> * features = [NSMutableArray array];
    ...
    for (double y = ceil(bounds.ne.latitude / gridSpacing) * gridSpacing; y >= floor(bounds.sw.latitude / gridSpacing) * gridSpacing; y -= gridSpacing) {
        ...
        VMGLPolylineFeature *feature = [VMGLPolylineFeature polylineWithCoordinates:coords count:2];
...
- (void)mapViewDidFinishRenderingFrame:(VMGLMapView *)mapView fullyRendered:(BOOL)fullyRendered {
    if (self.frameTimeGraphEnabled) {
        [self.frameTimeGraphView updatePathWithFrameDuration:mapView.frameTime];
    }
}

- (void)mapView:(nonnull VMGLMapView *)mapView didChangeLocationManagerAuthorization:(nonnull id<VMGLLocationManager>)manager {
    if (@available(iOS 14, *)) {
...
- (void)saveCurrentMapState:(__unused NSNotification *)notification {
    ...
    [[VMStateManager sharedManager] saveState:self.currentState];
}

- (void)restoreMapState:(__unused NSNotification *)notification {
    VMState *currentState = [VMStateManager sharedManager].currentState;
