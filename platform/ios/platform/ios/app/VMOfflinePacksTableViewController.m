...
static NSString * const VMOfflinePacksTableViewInactiveCellReuseIdentifier = @"Inactive";
static NSString * const VMOfflinePacksTableViewActiveCellReuseIdentifier = @"Active";
...
@implementation VMGLTilePyramidOfflineRegion (MBXAdditions)

- (void)applyToMapView:(VMGLMapView *)mapView {
...
@implementation VMOfflinePacksTableViewController

- (void)viewDidLoad {
    [super viewDidLoad];

    [[VMGLOfflineStorage sharedOfflineStorage] addObserver:self forKeyPath:@"packs" options:NSKeyValueObservingOptionInitial context:NULL];
...
- (void)dealloc {
    [[VMGLOfflineStorage sharedOfflineStorage] removeObserver:self forKeyPath:@"packs"];
...
- (IBAction)addCurrentRegion:(id)sender {
    ...
    [alertController addTextFieldWithConfigurationHandler:^(UITextField * _Nonnull textField) {
        textField.placeholder = [NSString stringWithFormat:@"%@", VMGLStringFromCoordinateBounds(self.mapView.visibleCoordinateBounds)];
    }];
    ...
    UIAlertAction *downloadAction = [UIAlertAction actionWithTitle:@"Download" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
        VMGLMapView *mapView = self.mapView;
        ...
        VMGLTilePyramidOfflineRegion *region = [[VMGLTilePyramidOfflineRegion alloc] initWithStyleURL:mapView.styleURL bounds:mapView.visibleCoordinateBounds fromZoomLevel:mapView.zoomLevel toZoomLevel:mapView.maximumZoomLevel];
        ...
        [[VMGLOfflineStorage sharedOfflineStorage] addPackForRegion:region withContext:context completionHandler:^(MGLOfflinePack *pack, NSError *error) {
            if (error) {
...
- (IBAction)invalidatePacks:(id)sender {
    for (MGLOfflinePack *pack in [VMGLOfflineStorage sharedOfflineStorage].packs) {
        ...
        [[VMGLOfflineStorage sharedOfflineStorage] invalidatePack:pack withCompletionHandler:^(NSError * _Nullable error) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [VMGLOfflineStorage sharedOfflineStorage].packs.count;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    MGLOfflinePack *pack = [VMGLOfflineStorage sharedOfflineStorage].packs[indexPath.row];

    NSString *reuseIdentifier = pack.state == MGLOfflinePackStateActive ? VMOfflinePacksTableViewActiveCellReuseIdentifier : VMOfflinePacksTableViewInactiveCellReuseIdentifier;
...
- (void)offlinePackProgressDidChange:(NSNotification *)notification {
    ...
    NSUInteger index = [[VMGLOfflineStorage sharedOfflineStorage].packs indexOfObject:pack];
    if (index == NSNotFound) {
        return;
    }
