...
const CGFloat VMUserLocationDotSize = 10;

@implementation VMUserLocationAnnotationView
...
- (CGSize)intrinsicContentSize
{
    ...
    return (self.mapView.userTrackingMode == VMGLUserTrackingModeFollowWithCourse) ? carSize : [self dotSize];
}
...
{
    (self.mapView.userTrackingMode == VMGLUserTrackingModeFollowWithCourse) ? [self drawCar] : [self drawDot];
}
...
- (void)drawDot
{
    ...
    // Dot
    center = self.bounds.size.width / 2.0 - VMUserLocationDotSize / 2.0;
    UIBezierPath *ovalPath = [UIBezierPath bezierPathWithOvalInRect: CGRectMake(center, center, VMUserLocationDotSize, VMUserLocationDotSize)];
