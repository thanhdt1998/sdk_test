...
@protocol VMGLCalloutViewDelegate;
...
/**
 ...
 `MGLCalloutView`.
 */
@protocol VMGLCalloutView <NSObject>

/**
 An object conforming to the `MGLAnnotation` protocol whose details this callout
 view displays.
 */
@property (nonatomic, strong) id <VMGLAnnotation> representedObject;
 ...
 */
@property (nonatomic, weak) id<VMGLCalloutViewDelegate> delegate;
 ...
 */
@protocol VMGLCalloutViewDelegate <NSObject>
 ...
 */
- (BOOL)calloutViewShouldHighlight:(UIView<VMGLCalloutView> *)calloutView;

/**
 Tells the delegate that the callout view has been tapped.
 */
- (void)calloutViewTapped:(UIView<VMGLCalloutView> *)calloutView;
