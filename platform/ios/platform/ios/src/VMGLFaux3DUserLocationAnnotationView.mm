...
#import "VMGLUserLocationHeadingArrowLayer.h"
#import "VMGLUserLocationHeadingBeamLayer.h"
...
@implementation VMGLFaux3DUserLocationAnnotationView
{
...
- (void)update
{
    ...
    {
        CGFloat frameSize = (self.mapView.userTrackingMode == VMGLUserTrackingModeFollowWithCourse) ? MGLUserLocationAnnotationPuckSize : MGLUserLocationAnnotationDotSize;
...
- (void)drawPreciseLocationPuck {
    ...
    (self.mapView.userTrackingMode == VMGLUserTrackingModeFollowWithCourse) ? [self drawPuck] : [self drawDot];
    [self updatePitch];
...
- (void)updatePitch
{
    if (self.mapView.camera.pitch != _oldPitch)
    {
        ...
        CATransform3D t = CATransform3DRotate(CATransform3DIdentity, VMGLRadiansFromDegrees(self.mapView.camera.pitch), 1.0, 0, 0);
...
{
    CGFloat pitch = VMGLRadiansFromDegrees(self.mapView.camera.pitch);
...
- (void)drawPuck
{
    ...
    {
        _puckArrow.affineTransform = CGAffineTransformRotate(CGAffineTransformIdentity, -VMGLRadiansFromDegrees(self.mapView.direction - self.userLocation.location.course));
    }
...
- (void)drawDot
{
    ...
    //
    BOOL headingTrackingModeEnabled = self.mapView.userTrackingMode == VMGLUserTrackingModeFollowWithHeading;
    ...
    if (showHeadingIndicator)
    {
        ...
        {
            CGFloat rotation = -VMGLRadiansFromDegrees(self.mapView.direction - headingDirection);
