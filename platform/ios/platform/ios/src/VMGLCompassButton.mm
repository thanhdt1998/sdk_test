...
@implementation VMGLCompassButton

+ (instancetype)compassButtonWithMapView:(VMGLMapView *)mapView {
    return [[VMGLCompassButton alloc] initWithMapView:mapView];
}
...
- (void)updateCompassAnimated:(BOOL)animated {
    ...
    self.transform = CGAffineTransformMakeRotation(VMGLRadiansFromDegrees(plateDirection));
...
- (void)animateToAlpha:(CGFloat)alpha {
    [UIView animateWithDuration:VMGLAnimationDuration delay:0 options:UIViewAnimationOptionBeginFromCurrentState animations:^{
         self.alpha = alpha;
     } completion:nil];
}
