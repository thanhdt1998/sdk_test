...
@interface VMGLAnnotationView () <UIGestureRecognizerDelegate>
...
@implementation VMGLAnnotationView
...
- (instancetype)initWithAnnotation:(nullable id<VMGLAnnotation>)annotation reuseIdentifier:(nullable NSString *)reuseIdentifier {
...
- (void)commonInitWithAnnotation:(nullable id<VMGLAnnotation>)annotation reuseIdentifier:(nullable NSString *)reuseIdentifier {
...
{
    MGLLogDebug(@"Setting selected: %@", VMGLStringFromBOOL(selected));
    [self setSelected:selected animated:NO];
}

- (void)setSelected:(BOOL)selected animated:(BOOL)animated
{
    MGLLogDebug(@"Setting selected: %@ animated: %@", VMGLStringFromBOOL(selected), VMGLStringFromBOOL(animated));
...
{
    MGLLogDebug(@"Setting scaleWithViewingDistance: %@", VMGLStringFromBOOL(scalesWithViewingDistance));
...
- (void)updateScaleTransformForViewingDistance
{
    ...
    if (superviewHeight > 0.0) {
        ...
        CGFloat pitchIntensity = pitch / VMGLDegreesFromRadians(mbgl::util::PITCH_MAX);
...
{
    MGLLogDebug(@"Setting rotatesToMatchCamera: %@", VMGLStringFromBOOL(rotatesToMatchCamera));
...
- (void)updateRotateTransform
{
    ...
    CGFloat direction = -VMGLRadiansFromDegrees(self.mapView.direction);
...
{
    MGLLogDebug(@"Setting draggable: %@", VMGLStringFromBOOL(draggable));
...
{
    MGLLogDebug(@"Setting dragState: %lu animated: %@", (unsigned long)dragState, VMGLStringFromBOOL(animated));
