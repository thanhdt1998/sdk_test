...
- (BOOL)isEqual:(id)other {
    return self == other || ([other isKindOfClass:[VMGLPolyline class]] && [super isEqual:other]);
}
        ...
        for (NSUInteger i = 0; i < count; i++) {
            ...
            if (traveled >= middle) {
                ...
                MGLRadianCoordinate2D otherCoordinate = MGLRadianCoordinateAtDistanceFacingDirection(from,
                                                                                                     ...
                                                                                                     VMGLRadiansFromDegrees(direction));
                return CLLocationCoordinate2DMake(VMGLDegreesFromRadians(otherCoordinate.latitude),
                                                  VMGLDegreesFromRadians(otherCoordinate.longitude));
            }
...
{
    return VMGLDegreesFromRadians(MGLRadianCoordinatesDirection(from, to));
}

@end

@interface VMGLMultiPolyline ()

@property (nonatomic, copy, readwrite) NSArray<VMGLPolyline *> *polylines;

@end

@implementation VMGLMultiPolyline {
    VMGLCoordinateBounds _overlayBounds;
}

@synthesize overlayBounds = _overlayBounds;

+ (instancetype)multiPolylineWithPolylines:(NSArray<VMGLPolyline *> *)polylines {
    return [[self alloc] initWithPolylines:polylines];
}

- (instancetype)initWithPolylines:(NSArray<VMGLPolyline *> *)polylines {
    ...
    if (self = [super init]) {
        ...
        for (VMGLPolyline *polyline in _polylines) {
            bounds.extend(VMGLLatLngBoundsFromCoordinateBounds(polyline.overlayBounds));
        }
...
- (BOOL)isEqual:(id)other
{
    ...
    if (![other isKindOfClass:[VMGLMultiPolyline class]]) return NO;

    VMGLMultiPolyline *otherMultipoline = other;
...
- (NSUInteger)hash {
    ...
    for (VMGLPolyline *polyline in self.polylines) {
        hash += [polyline hash];
    }
    return hash;
}
...
- (CLLocationCoordinate2D)coordinate {
    VMGLPolyline *polyline = self.polylines.firstObject;
...
- (NSDictionary *)geoJSONDictionary {
    ...
    for (VMGLPolylineFeature *feature in self.polylines) {
