...
#import "VMGLShapeSource_Private.h"
...
- (instancetype)initWithIdentifier:(NSString *)identifier shape:(nullable VMGLShape *)shape options:(NSDictionary<MGLShapeSourceOption, id> *)options {
    auto geoJSONOptions = MGLGeoJSONOptionsFromDictionary(options);
    auto source = std::make_unique<mbgl::style::GeoJSONSource>(identifier.UTF8String, std::move(geoJSONOptions));
    if (self = [super initWithPendingSource:std::move(source)]) {
        if ([shape isMemberOfClass:[VMGLShapeCollection class]]) {
...
- (instancetype)initWithIdentifier:(NSString *)identifier features:(NSArray<VMGLShape<VMGLFeature> *> *)features options:(nullable NSDictionary<MGLShapeSourceOption, id> *)options {
    for (id <VMGLFeature> feature in features) {
        if (![feature conformsToProtocol:@protocol(VMGLFeature)]) {
...
- (instancetype)initWithIdentifier:(NSString *)identifier shapes:(NSArray<VMGLShape *> *)shapes options:(nullable NSDictionary<MGLShapeSourceOption, id> *)options {
    VMGLShapeCollection *shapeCollection = [VMGLShapeCollection shapeCollectionWithShapes:shapes];
...
- (NSArray<id <VMGLFeature>> *)featuresMatchingPredicate:(nullable NSPredicate *)predicate {
    MGLAssertStyleSourceIsValid();
    ...
    if ([self.stylable isKindOfClass:[VMGLMapView class]]) {
        VMGLMapView *mapView = (VMGLMapView *)self.stylable;
...
- (mbgl::optional<mbgl::FeatureExtensionValue>)featureExtensionValueOfCluster:(VMGLShape<MGLCluster> *)cluster extension:(std::string)extension options:(const std::map<std::string, mbgl::Value>)options {
    MGLAssertStyleSourceIsValid();
...
- (NSArray<id <VMGLFeature>> *)leavesOfCluster:(MGLPointFeatureCluster *)cluster offset:(NSUInteger)offset limit:(NSUInteger)limit {
...
- (NSArray<id <VMGLFeature>> *)childrenOfCluster:(MGLPointFeatureCluster *)cluster {
...
- (void)debugRecursiveLogForFeature:(id <VMGLFeature>)feature indent:(NSUInteger)indent {
    ...
    if (cluster) {
        for (id <VMGLFeature> child in [self childrenOfCluster:cluster]) {
