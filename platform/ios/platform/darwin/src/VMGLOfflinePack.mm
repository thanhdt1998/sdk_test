...
#import "VMGLShapeOfflineRegion_Private.h"
...
const VMGLExceptionName MGLInvalidOfflinePackException = @"MGLInvalidOfflinePackException";
...
- (instancetype)initWithMBGLRegion:(mbgl::OfflineRegion *)region {
    if (self = [super init]) {
        ...
        _mbglDatabaseFileSource = [[VMGLOfflineStorage sharedOfflineStorage] mbglDatabaseFileSource];
...
- (id <VMOfflineRegion>)region {
    MGLAssertOfflinePackIsValid();

    const mbgl::OfflineRegionDefinition &regionDefinition = _mbglOfflineRegion->getDefinition();
    MGLAssert([VMGLTilePyramidOfflineRegion  conformsToProtocol:@protocol(MGLOfflineRegion_Private)], @"MGLTilePyramidOfflineRegion should conform to MGLOfflineRegion_Private.");
    ...
    return regionDefinition.match(
                           [&] (const mbgl::OfflineTilePyramidRegionDefinition def){
                               return (id <VMOfflineRegion>)[[VMGLTilePyramidOfflineRegion  alloc] initWithOfflineRegionDefinition:def];
                           },
                           [&] (const mbgl::OfflineGeometryRegionDefinition& def){
                               return (id <VMOfflineRegion>)[[MGLShapeOfflineRegion alloc] initWithOfflineRegionDefinition:def];
                           });
}
...
- (void)reloadWithCompletionHandler:(void (^)(NSError * _Nullable error))completion {
    ...
    VMGLOfflineStorage *sharedOfflineStorage = [VMGLOfflineStorage sharedOfflineStorage];
    __weak MGLOfflinePack *weakSelf = self;
