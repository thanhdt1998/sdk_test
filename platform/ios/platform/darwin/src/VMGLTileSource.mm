...
- (NSArray<VMGLAttributionInfo *> *)attributionInfos {
...
- (NSArray<VMGLAttributionInfo *> *)attributionInfosWithFontSize:(CGFloat)fontSize linkColor:(nullable MGLColor *)linkColor {
    return [VMGLAttributionInfo attributionInfosFromHTMLString:self.attributionHTMLString
                                                     fontSize:fontSize
                                                    linkColor:linkColor];
}
...
mbgl::Tileset MGLTileSetFromTileURLTemplates(NSArray<NSString *> *tileURLTemplates, NSDictionary<MGLTileSourceOption, id> * _Nullable options) {
    ...
    if (NSValue *coordinateBounds = options[MGLTileSourceOptionCoordinateBounds]) {
        if (![coordinateBounds isKindOfClass:[NSValue class]]
            && strcmp(coordinateBounds.objCType, @encode(VMGLCoordinateBounds)) == 0) {
            [NSException raise:NSInvalidArgumentException
                        format:@"MGLTileSourceOptionCoordinateBounds must be set to an NSValue containing an MGLCoordinateBounds."];
        }
        tileSet.bounds = VMGLLatLngBoundsFromCoordinateBounds(coordinateBounds.MGLCoordinateBoundsValue);
    }
    ...
    if (NSArray *attributionInfos = options[MGLTileSourceOptionAttributionInfos]) {
        ...
        NSAttributedString *attributedString = [VMGLAttributionInfo attributedStringForAttributionInfos:attributionInfos];
#if TARGET_OS_IPHONE
