...
+ (instancetype)sharedSettings {
    ...
    static VMGLSettings *_sharedSettings;
...
+ (void)setApiKey:(NSString *)apiKey {
    ...
    [VMGLSettings sharedSettings].apiKey = apiKey;
}

+ (NSString *)apiKey {
    return [VMGLSettings sharedSettings].apiKey;
}
...
+ (void)setAPIBaseURL:(NSURL *)apiBaseURL {
    ...
    [VMGLSettings sharedSettings].tileServerOptionsInternal = &tileServerOptions->withBaseURL(baseUrl);
}

+ (NSURL *)apiBaseURL {
    auto baseUrl = [VMGLSettings sharedSettings].tileServerOptionsInternal->baseURL();
...
+ (void)setTileServerOptionsInternal:(mbgl::TileServerOptions)options {
    [VMGLSettings sharedSettings].tileServerOptionsInternal = new mbgl::TileServerOptions();
    *[VMGLSettings sharedSettings].tileServerOptionsInternal = options.clone();
    [self tileServerOptionsChanged];
}

+ (mbgl::TileServerOptions)tileServerOptionsInternal {
    auto options = [VMGLSettings sharedSettings];
    return options.tileServerOptionsInternal->clone();
}

+ (void)tileServerOptionsChanged {
    [VMGLSettings sharedSettings].tileServerOptionsChangeToken = [[NSUUID UUID] UUIDString];
}

+ (void)useWellKnownTileServer:(MGLWellKnownTileServer)tileServer {
    switch(tileServer){
        case MGLMapTiler:
            [VMGLSettings setTileServerOptionsInternal:mbgl::TileServerOptions::MapTilerConfiguration()];
            break;
        case MGLMapLibre:
            [VMGLSettings setTileServerOptionsInternal:mbgl::TileServerOptions::MapLibreConfiguration()];
            break;
        case MGLMapbox:
            [VMGLSettings setTileServerOptionsInternal:mbgl::TileServerOptions::MapboxConfiguration()];
            break;
        default:
            [VMGLSettings setTileServerOptionsInternal:mbgl::TileServerOptions::DefaultConfiguration()];
    }
}
...
+ (MGLTileServerOptions*)tileServerOptions {
    auto cppOpts = [VMGLSettings sharedSettings].tileServerOptionsInternal;
    if (cppOpts) {
