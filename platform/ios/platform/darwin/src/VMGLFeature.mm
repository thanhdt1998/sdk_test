...
#import "VMGLPointCollection_Private.h"
#import "VMGLPolyline_Private.h"
...
@interface VMGLPolylineFeature ()
@end

@implementation VMGLPolylineFeature
...
@interface VMGLMultiPolylineFeature ()
@end

@implementation VMGLMultiPolylineFeature
...
+ (instancetype)shapeCollectionWithShapes:(NSArray<VMGLShape<VMGLFeature> *> *)shapes {
...
- (mbgl::GeoJSON)geoJSONObject {
    ...
    for (VMGLShape <VMGLFeature> *feature in self.shapes) {
...
public:
    ...
    VMGLShape * operator()(const mbgl::EmptyGeometry &) const {
        return is_in_feature ? [[MGLEmptyFeature alloc] init] : [[VMGLShape alloc] init];
    }

    VMGLShape * operator()(const mbgl::Point<T> &geometry) const {
        Class shapeClass = is_in_feature ? [MGLPointFeature class] : [VMPointAnnotation class];
        ...
        VMPointAnnotation *shape = [[shapeClass alloc] init];
        shape.coordinate = toLocationCoordinate2D(geometry);
        return shape;
    }

    VMGLShape * operator()(const mbgl::LineString<T> &geometry) const {
        std::vector<CLLocationCoordinate2D> coordinates = toLocationCoordinates2D(geometry);
        Class shapeClass = is_in_feature ? [VMGLPolylineFeature class] : [VMGLPolyline class];
        return [shapeClass polylineWithCoordinates:&coordinates[0] count:coordinates.size()];
    }

    VMGLShape * operator()(const mbgl::Polygon<T> &geometry) const {
        return toShape<VMGLPolygon, MGLPolygonFeature>(geometry, is_in_feature);
    }

    VMGLShape * operator()(const mbgl::MultiPoint<T> &geometry) const {
        std::vector<CLLocationCoordinate2D> coordinates = toLocationCoordinates2D(geometry);
        Class shapeClass = is_in_feature ? [MGLPointCollectionFeature class] : [VMGLPointCollection class];
        return [[shapeClass alloc] initWithCoordinates:&coordinates[0] count:coordinates.size()];
    }

    VMGLShape * operator()(const mbgl::MultiLineString<T> &geometry) const {
        ...
        for (auto &lineString : geometry) {
            ...
            VMGLPolyline *polyline = [VMGLPolyline polylineWithCoordinates:&coordinates[0] count:coordinates.size()];
            [polylines addObject:polyline];
        }

        Class shapeClass = is_in_feature ? [VMGLMultiPolylineFeature class] : [VMGLMultiPolyline class];
        return [shapeClass multiPolylineWithPolylines:polylines];
    }

    VMGLShape * operator()(const mbgl::MultiPolygon<T> &geometry) const {
        NSMutableArray *polygons = [NSMutableArray arrayWithCapacity:geometry.size()];
        for (auto &polygon : geometry) {
            [polygons addObject:toShape(polygon, false)];
        }

        Class shapeClass = is_in_feature ? [MGLMultiPolygonFeature class] : [VMGLMultiPolygon class];
        return [shapeClass multiPolygonWithPolygons:polygons];
    }

    VMGLShape * operator()(const mapbox::geometry::geometry_collection<T> &collection) const {
        ...
        for (auto &geometry : collection) {
            ...
            VMGLShape *shape = mapbox::geometry::geometry<T>::visit(geometry, *this);
            [shapes addObject:shape];
        }
        Class shapeClass = is_in_feature ? [MGLShapeCollectionFeature class] : [VMGLShapeCollection class];
...
private:
    ...
    template<typename U = VMGLPolygon, typename V = MGLPolygonFeature>
    static U *toShape(const mbgl::Polygon<T> &geometry, const bool isInFeature) {
        ...
        if (geometry.size() > 1) {
            ...
            for (auto iter = geometry.begin() + 1; iter != geometry.end(); iter++) {
                ...
                VMGLPolygon *innerPolygon = [VMGLPolygon polygonWithCoordinates:&innerCoordinates[0] count:innerCoordinates.size()];
...
public:
    VMGLShape * operator()(const mbgl::Geometry<T> &geometry) const {
        GeometryEvaluator<T> evaluator;
        VMGLShape *shape = mapbox::geometry::geometry<T>::visit(geometry, evaluator);
        return shape;
    }

    VMGLShape <VMGLFeature> * operator()(const mbgl::GeoJSONFeature &feature) const {
        VMGLShape <VMGLFeature> *shape = (VMGLShape <VMGLFeature> *)VMGLFeatureFromMBGLFeature(feature);
        return shape;
    }

    VMGLShape <VMGLFeature> * operator()(const mbgl::FeatureCollection &collection) const {
        NSMutableArray *shapes = [NSMutableArray arrayWithCapacity:collection.size()];
        for (const auto &feature : collection) {
            [shapes addObject:VMGLFeatureFromMBGLFeature(feature)];
        }
        return [MGLShapeCollectionFeature shapeCollectionWithShapes:shapes];
    }
};

NSArray<VMGLShape <VMGLFeature> *> *MGLFeaturesFromMBGLFeatures(const std::vector<mbgl::Feature> &features) {
    NSMutableArray *shapes = [NSMutableArray arrayWithCapacity:features.size()];
    for (const auto &feature : features) {
        [shapes addObject:VMGLFeatureFromMBGLFeature(static_cast<mbgl::GeoJSONFeature>(feature))];
    }
    return shapes;
}

NSArray<VMGLShape <VMGLFeature> *> *MGLFeaturesFromMBGLFeatures(const std::vector<mbgl::GeoJSONFeature> &features) {
    NSMutableArray *shapes = [NSMutableArray arrayWithCapacity:features.size()];
    for (const auto &feature : features) {
        [shapes addObject:VMGLFeatureFromMBGLFeature(feature)];
    }
...
id <VMGLFeature> VMGLFeatureFromMBGLFeature(const mbgl::GeoJSONFeature &feature) {
    ...
    VMGLShape <VMGLFeature> *shape = (VMGLShape <VMGLFeature> *)mapbox::geometry::geometry<double>::visit(feature.geometry, evaluator);
...
VMGLShape* VMGLShapeFromGeoJSON(const mapbox::geojson::geojson &geojson) {
    GeoJSONEvaluator<double> evaluator;
    VMGLShape *shape = mapbox::geojson::geojson::visit(geojson, evaluator);
    return shape;
}
