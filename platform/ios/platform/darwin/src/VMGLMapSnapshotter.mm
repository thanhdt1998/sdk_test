...
#import "VMGLMapSnapshotter_Private.h"
...
NSArray<VMGLAttributionInfo *> *MGLAttributionInfosFromAttributions(mbgl::MapSnapshotter::Attributions attributions);
...
{
    mbgl::ScreenCoordinate sc = _pointForFn(VMGLLatLngFromLocationCoordinate2D(coordinate));
...
- (CLLocationCoordinate2D)coordinateForPoint:(CGPoint)point
{
    ...
    return VMGLLocationCoordinate2DFromLatLng(latLng);
}
...
- (instancetype _Nonnull)initWithStyleURL:(nullable NSURL *)styleURL camera:(VMGLMapCamera *)camera size:(CGSize)size
{
...
- (void)startWithQueue:(dispatch_queue_t)queue overlayHandler:(MGLMapSnapshotOverlayHandler)overlayHandler completionHandler:(MGLMapSnapshotCompletionHandler)completion
{
    ...
    if ([NSThread.currentThread isMainThread]) {
        (void)[VMGLOfflineStorage sharedOfflineStorage];
    } else {
...
MGLImage *MGLAttributedSnapshot(mbgl::MapSnapshotter::Attributions attributions, MGLImage *mglImage, MGLMapSnapshotOptions *options, void (^overlayHandler)()) {
    ...
    NSArray<VMGLAttributionInfo *> *attributionInfo = MGLAttributionInfosFromAttributions(attributions);
...
NSArray<VMGLAttributionInfo *> *MGLAttributionInfosFromAttributions(mbgl::MapSnapshotter::Attributions attributions) {
    ...
    for (auto attribution : attributions) {
        ...
        NSArray *tileSetInfos = [VMGLAttributionInfo attributionInfosFromHTMLString:attributionHTMLString
                                                                          fontSize:fontSize
...
+ (void)drawAttributionTextWithStyle:(MGLAttributionInfoStyle)attributionInfoStyle origin:(CGPoint)origin attributionInfo:(NSArray<VMGLAttributionInfo *>*)attributionInfo
{
    for (VMGLAttributionInfo *info in attributionInfo) {
...
+ (CGSize)attributionSizeWithLogoStyle:(MGLAttributionInfoStyle)logoStyle sourceAttributionStyle:(MGLAttributionInfoStyle)attributionStyle attributionInfo:(NSArray<VMGLAttributionInfo *>*)attributionInfo
{
...
+ (CGSize)attributionTextSizeWithStyle:(MGLAttributionInfoStyle)attributionStyle attributionInfo:(NSArray<VMGLAttributionInfo *>*)attributionInfo
{
...
- (void)configureWithOptions:(MGLMapSnapshotOptions *)options {
    auto mbglFileSource = [[VMGLOfflineStorage sharedOfflineStorage] mbglFileSource];
    ...
    auto tileServerOptions = [[VMGLSettings sharedSettings] tileServerOptionsInternal];
    mbgl::ResourceOptions resourceOptions;
    resourceOptions
        .withTileServerOptions(*tileServerOptions)
        .withCachePath(VMGLOfflineStorage.sharedOfflineStorage.databasePath.UTF8String)
    ...
    if (CLLocationCoordinate2DIsValid(options.camera.centerCoordinate)) {
        cameraOptions.center = VMGLLatLngFromLocationCoordinate2D(options.camera.centerCoordinate);
    }
    ...
    if (!MGLCoordinateBoundsIsEmpty(options.coordinateBounds)) {
        _mbglMapSnapshotter->setRegion(VMGLLatLngBoundsFromCoordinateBounds(options.coordinateBounds));
    }
}
...
- (VMGLStyle *)style {
    ...
    return [[VMGLStyle alloc] initWithRawStyle:&_mbglMapSnapshotter->getStyle() stylable:self];
}
