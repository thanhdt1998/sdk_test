...
#import "VMGLOfflinePack_Private.h"
...
const VMGLExceptionName MGLUnsupportedRegionTypeException = @"MGLUnsupportedRegionTypeException";

@interface VMGLOfflineStorage ()
...
@implementation VMGLOfflineStorage {
    NSURL *_databaseURL;
...
+ (instancetype)sharedOfflineStorage {
    ...
    static VMGLOfflineStorage *sharedOfflineStorage;
...
- (instancetype)init {
    ...
    if (self = [super init]) {
        ...
        [[VMGLSettings sharedSettings] addObserver:self
...
- (void)dealloc {
    ...
    [[VMGLSettings sharedSettings] removeObserver:self forKeyPath:@"tileServerOptionsChangeToken"];
    [[VMGLSettings sharedSettings] removeObserver:self forKeyPath:@"apiKey"];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *, id> *)change context:(void *)context {
    ...
    if ([keyPath isEqualToString:@"apiKey"] && object == [VMGLSettings sharedSettings]) {
    ...
    } else if ([keyPath isEqualToString:@"tileServerOptionsChangeToken"] && object == [VMGLSettings sharedSettings]) {
...
- (void)addContentsOfURL:(NSURL *)fileURL withCompletionHandler:(MGLBatchedOfflinePackAdditionCompletionHandler)completion {
    ...
    __weak VMGLOfflineStorage *weakSelf = self;
    [self _addContentsOfFile:fileURL.path withCompletionHandler:^(NSArray<MGLOfflinePack *> * _Nullable packs, NSError * _Nullable error) {
        if (packs) {
            ...
            VMGLOfflineStorage *strongSelf = weakSelf;
...
- (void)addPackForRegion:(id <VMOfflineRegion>)region withContext:(NSData *)context completionHandler:(MGLOfflinePackAdditionCompletionHandler)completion {
...
- (void)_addPackForRegion:(id <VMOfflineRegion>)region withContext:(NSData *)context completionHandler:(MGLOfflinePackAdditionCompletionHandler)completion {
...
- (void)setMaximumAllowedMapboxTiles:(uint64_t)maximumCount {
    ...
    _mbglDatabaseFileSource->setOfflineVietmapTileCountLimit(maximumCount);
}
...
- (void)preloadData:(NSData *)data forURL:(NSURL *)url modificationDate:(nullable NSDate *)modified expirationDate:(nullable NSDate *)expires eTag:(nullable NSString *)eTag mustRevalidate:(BOOL)mustRevalidate
    ...
    if (modified) {
        response.modified = mbgl::Timestamp() + std::chrono::duration_cast<mbgl::Seconds>(VMGLDurationFromTimeInterval(modified.timeIntervalSince1970));
    }
    
    if (expires) {
        response.expires = mbgl::Timestamp() + std::chrono::duration_cast<mbgl::Seconds>(VMGLDurationFromTimeInterval(expires.timeIntervalSince1970));
    }
