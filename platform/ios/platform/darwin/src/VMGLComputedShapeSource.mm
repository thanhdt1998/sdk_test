#import "VMGLComputedShapeSource_Private.h"
...
const VMGLExceptionName MGLInvalidDatasourceException = @"MGLInvalidDatasourceException";
...
@property (nonatomic, assign) BOOL dataSourceImplementsFeaturesForTile;
@property (nonatomic, assign) BOOL dataSourceImplementsFeaturesForBounds;
@property (nonatomic, weak, nullable) id<VMComputedShapeSourceDataSource> dataSource;
...
- (void)main {
    ...
    NSArray<VMGLShape <VMGLFeature> *> *data;
    if(!self.dataSource) {
        data = nil;
        ...
        data = [self.dataSource featuresInCoordinateBounds:VMGLCoordinateBoundsFromLatLngBounds(tileBounds)
                                                 zoomLevel:self.z];
    }
    ...
    if(![self isCancelled]) {
        ...
        for (VMGLShape <VMGLFeature> * feature in data) {
            if ([feature isMemberOfClass:[VMGLShapeCollection class]]) {
...
- (instancetype)initWithIdentifier:(NSString *)identifier dataSource:(id<VMComputedShapeSourceDataSource>)dataSource options:(NSDictionary<MGLShapeSourceOption, id> *)options {
...
- (void)setFeatures:(NSArray<VMGLShape <VMGLFeature> *>*)features inTileAtX:(NSUInteger)x y:(NSUInteger)y zoomLevel:(NSUInteger)zoomLevel {
    ...
    for (VMGLShape <VMGLFeature> * feature in features) {
        mbgl::GeoJSONFeature geoJsonObject = [feature geoJSONObject].get<mbgl::GeoJSONFeature>();
        featureCollection.push_back(geoJsonObject);
        if ([feature isMemberOfClass:[VMGLShapeCollection class]]) {
            static dispatch_once_t onceToken;
...
- (void)setDataSource:(id<VMComputedShapeSourceDataSource>)dataSource {
    [self.requestQueue cancelAllOperations];
...
- (void) invalidateBounds:(VMGLCoordinateBounds)bounds {
    MGLAssertStyleSourceIsValid();
    ((mbgl::style::CustomGeometrySource *)self.rawSource)->invalidateRegion(VMGLLatLngBoundsFromCoordinateBounds(bounds));
}
