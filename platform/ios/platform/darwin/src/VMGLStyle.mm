...
#import "VMGLStyleLayerManager.h"

#import "VMGLSource.h"
#import "VMGLSource_Private.h"
#import "VMGLLight_Private.h"
#import "VMGLTileSource_Private.h"
...
const VMGLExceptionName MGLInvalidStyleURLException = @"MGLInvalidStyleURLException";
const VMGLExceptionName MGLRedundantLayerException = @"MGLRedundantLayerException";
const VMGLExceptionName MGLRedundantLayerIdentifierException = @"MGLRedundantLayerIdentifierException";
const VMGLExceptionName MGLRedundantSourceException = @"MGLRedundantSourceException";
const VMGLExceptionName MGLRedundantSourceIdentifierException = @"MGLRedundantSourceIdentifierException";
...
@implementation VMGLStyle

#pragma mark Predefined style URLs

+ (NSArray<MGLDefaultStyle*>*) predefinedStyles {
    return VMGLSettings.tileServerOptions.defaultStyles;
}

+ (MGLDefaultStyle*) defaultStyle {
    MGLTileServerOptions* opts = VMGLSettings.tileServerOptions;
    return opts.defaultStyle;
}

+ (NSURL*) defaultStyleURL {
    MGLDefaultStyle* styleDefinition = [VMGLStyle defaultStyle];
...
+ (MGLDefaultStyle*) predefinedStyle:(NSString*)withStyleName {
    for (MGLDefaultStyle* style in VMGLSettings.tileServerOptions.defaultStyles) {
...
- (NSSet<__kindof VMGLSource *> *)sources {
    auto rawSources = self.rawStyle->getSources();
    NSMutableSet<__kindof VMGLSource *> *sources = [NSMutableSet setWithCapacity:rawSources.size()];
    for (auto rawSource = rawSources.begin(); rawSource != rawSources.end(); ++rawSource) {
        VMGLSource *source = [self sourceFromMBGLSource:*rawSource];
        [sources addObject:source];
    }
    return sources;
}

- (void)setSources:(NSSet<__kindof VMGLSource *> *)sources {
    MGLLogDebug(@"Setting: %lu sources", sources.count);
    for (VMGLSource *source in self.sources) {
        [self removeSource:source];
    }
    for (VMGLSource *source in sources) {
...
- (VMGLSource *)memberOfSources:(VMGLSource *)object {
    return [self sourceWithIdentifier:object.identifier];
}

- (VMGLSource *)sourceWithIdentifier:(NSString *)identifier
{
...
- (VMGLSource *)sourceFromMBGLSource:(mbgl::style::Source *)rawSource {
    if (VMGLSource *source = rawSource->peer.has_value() ? rawSource->peer.get<SourceWrapper>().source : nil) {
        return source;
    }
    ...
    if (auto vectorSource = rawSource->as<mbgl::style::VectorSource>()) {
        ...
        return [[VMGLSource alloc] initWithRawSource:rawSource stylable:self.stylable];
    }
}
