...
@interface VMGLNetworkConfiguration () <MGLNativeNetworkDelegate>
...
@implementation VMGLNetworkConfiguration
{
...
+ (instancetype)sharedManager {
    ...
    static VMGLNetworkConfiguration *_sharedManager;
...
- (void)setSessionConfiguration:(NSURLSessionConfiguration *)sessionConfiguration {
    @synchronized (self) {
        if (sessionConfiguration == nil) {
            _sessionConfig = [VMGLNetworkConfiguration defaultSessionConfiguration];
        } else {
...
- (NSDictionary *)eventAttributesForURL:(NSURLResponse *)response withAction:(NSString *)action
{
    ...
    BOOL isWIFIOn = [[VMGLReachability reachabilityWithHostName:response.URL.host] isReachableViaWiFi];
...
@implementation VMGLNetworkConfiguration (ForTesting)
