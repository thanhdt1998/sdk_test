...
@property (nonatomic, readonly, nullable) VMGLMapView *mapView;
...
/**
 ...
 set to `nil`.
 */
@property (nonatomic, weak, readwrite) VMGLStyle *style;
...
- (VMGLMapView *)mapView {
    if ([self.style.stylable isKindOfClass:[VMGLMapView class]]) {
        return (VMGLMapView *)self.style.stylable;
    }
    return nil;
}
...
- (void)addToStyle:(VMGLStyle *)style belowLayer:(MGLStyleLayer *)otherLayer {
    self.style = style;
    self.style.openGLLayers[self.identifier] = self;
    [super addToStyle:style belowLayer:otherLayer];
}

- (void)removeFromStyle:(VMGLStyle *)style {
