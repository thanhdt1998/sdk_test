                                    ...
                                    eyeAltitude:(CLLocationDistance)eyeAltitude
{
    ...
    if (CLLocationCoordinate2DIsValid(centerCoordinate) && CLLocationCoordinate2DIsValid(eyeCoordinate)) {
        ...
        pitch = mbgl::util::wrap(90 - VMGLDegreesFromRadians(radianPitch), 0.0, 360.0);
    }
                                        ...
                                        heading:(CLLocationDirection)heading
{
    ...
    CLLocationDistance altitude = distance * sin(VMGLRadiansFromDegrees(eyeAngle));
                                        ...
                                        heading:(CLLocationDirection)heading
{
    ...
    return [VMGLMapCamera cameraLookingAtCenterCoordinate:centerCoordinate
                                                altitude:distance
                                                   pitch:pitch
                                                 heading:heading];
}
...
- (instancetype)initWithCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
                                altitude:(CLLocationDistance)altitude
                                 ...
                                 heading:(CLLocationDirection)heading
{
    MGLLogDebug(@"Initializing withCenterCoordinate: %@ altitude: %.0fm pitch: %f° heading: %f°", VMGLStringFromCLLocationCoordinate2D(centerCoordinate), altitude, pitch, heading);
    if (self = [super init])
    {
...
- (CLLocationDistance)viewingDistance {
    ...
    return self.altitude / sin(VMGLRadiansFromDegrees(eyeAngle));
}

- (void)setViewingDistance:(CLLocationDistance)distance {
    MGLLogDebug(@"Setting viewingDistance: %f", distance);
    CLLocationDirection eyeAngle = 90 - self.pitch;
    self.altitude = distance * sin(VMGLRadiansFromDegrees(eyeAngle));
}
...
- (BOOL)isEqual:(id)other
{
    ...
    VMGLMapCamera *otherCamera = other;
...
- (BOOL)isEqualToMapCamera:(VMGLMapCamera *)otherCamera
{
    if (otherCamera == self)
    {
        return YES;
