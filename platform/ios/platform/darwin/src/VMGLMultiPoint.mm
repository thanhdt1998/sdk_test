...
- (BOOL)isEqual:(id)other
{
    ...
    if (![other isKindOfClass:[VMGLMultiPoint class]]) return NO;

    VMGLMultiPoint *otherMultipoint = other;
...
- (VMGLCoordinateBounds)overlayBounds
{
    if (!_bounds) {
        ...
        for (auto coordinate : _coordinates) {
            ...
            bounds.extend(VMGLLatLngFromLocationCoordinate2D(coordinate));
        }
        _bounds = bounds;
    }
    return VMGLCoordinateBoundsFromLatLngBounds(*_bounds);
}
...
- (mbgl::Annotation)annotationObjectWithDelegate:(__unused id <VMGLMultiPointDelegate>)delegate
{
